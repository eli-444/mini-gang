  ...
  return (
    <form onSubmit={submit} className="grid gap-3">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        ...
        <input value={form.phone} onChange={(event) => setForm((prev) => ({ ...prev, phone: event.target.value }))} placeholder="Téléphone" className="rounded-xl border border-[var(--mg-ring)] px-3 py-2 text-sm" />
        ...
        <input value={form.iban_last4} onChange={(event) => setForm((prev) => ({ ...prev, iban_last4: event.target.value }))} placeholder="IBAN 4 derniers caractères" className="rounded-xl border border-[var(--mg-ring)] px-3 py-2 text-sm" maxLength={4} />
        <input value={form.iban_encrypted} onChange={(event) => setForm((prev) => ({ ...prev, iban_encrypted: event.target.value }))} placeholder="IBAN (MVP manuel, champ sécurisé plus tard)" className="rounded-xl border border-[var(--mg-ring)] px-3 py-2 text-sm md:col-span-2" />
      </div>
