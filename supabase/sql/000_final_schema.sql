-- Le Mini Gang - schema final Suisse.
-- ATTENTION: ce fichier remet a zero le schema public.
-- A utiliser sur une base neuve ou apres sauvegarde.
...
create index vetements_reserved_until_idx on public.vetements(reserved_until) where statut = 'reserve';
...
create table public.commandes (
  ...
  sous_total_centimes integer not null default 0 check (sous_total_centimes >= 0),
  frais_livraison_centimes integer not null default 0 check (frais_livraison_centimes >= 0),
  ...
  payment_provider text check (payment_provider in ('stripe', 'klarna', 'twint') or payment_provider is null),
...
create unique index commandes_provider_session_id_unique on public.commandes(provider_session_id) where provider_session_id is not null;
create unique index commandes_provider_payment_id_unique on public.commandes(provider_payment_id) where provider_payment_id is not null;
...
create table public.admin_settings (
  ...
  shipping_fee_cents integer not null default 790 check (shipping_fee_cents >= 0),
...
create trigger maj_admin_settings_modifie_le
...
-- Apres inscription, passer ton compte admin:
